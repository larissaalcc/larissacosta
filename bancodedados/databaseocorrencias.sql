...
select * from ms where qtde >3;
select * from ms where vitima >5;
select * from ms where vitima =0;
select * from ms where municipio = 'brasilandia';
select * from ms order by municipio desc;
select max(vitima) from ms; #encotra o valor máximo na tabela 
select count(vitima) from ms where vitima = 1;
select sum(vitima) from ms;
select * from ms where vitima between 0 and 9;
